...
inline int get_enum_value(std::map<int, std::string>& values)
{
		...
		&& enumStr[start] != '(')
	{
		...
		values.insert({static_cast<int>(enumValue), enumStr});
	}
...
{
	auto dummy = {get_enum_value<Enum, DummyFlag, static_cast<Enum>(Ints + min_value)>(values)...};
}
...
{
	auto dummy = {
		get_enum_value<Enum, DummyFlag, static_cast<Enum>(0)>(values),
		get_enum_value<Enum, DummyFlag, static_cast<Enum>(1 << (int)Ints)>(values)...
	};
}
